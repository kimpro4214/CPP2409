...
int main() {
    vector<vector<int>> map(map_y, vector<int>(map_x));

    // 맵을 파일에서 불러오기, 예외처리
    ifstream is{"map.txt"};
    if (!is) {
        throw runtime_error("맵 파일을 열 수 없습니다: map.txt");
    }
    ...
    try {
        for (int i = 0; i < map_y; ++i) {
            for (int j = 0; j < map_x; ++j) {
                is >> map[i][j];
                if (is.fail()) {
                    throw runtime_error("맵 데이터를 읽는 중 오류가 발생했습니다.");
                }
            }
        }
    ...
    bool is_gameover = false;
        ...
        if (choice != 1 && choice != 2) {
            throw out_of_range("1 또는 2만 선택할 수 있습니다.");
        }
